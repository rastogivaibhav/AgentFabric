...
printf '%s\n' \
  ...
  "a13_context_policy=head_tail_deterministic_bound" \
  "a13_transport_policy=single_inflight_extended_local_timeout" \
  "a13_validity_gate=llm_errors_zero_and_transport_clean" > "$OUT_DIR/treatment_contract.txt"
...
OUT_DIR="$OUT_DIR" PROMPT_BUDGET_CHARS="$PROMPT_BUDGET_CHARS" CLIENT_TIMEOUT_FLOOR_S="$CLIENT_TIMEOUT_FLOOR_S" python3 - <<'PY'
...
assert eligible > 0, (eligible, raised, original)
...
metrics = {
    ...
    'max_total_s': max(float(r.get('total_elapsed_s', 0)) for r in posts),
...
json.dump(metrics, open(out / 'transport_metrics.json', 'w'), indent=2, sort_keys=True)
print('pi45a13_transport_contract=PASS')
print(json.dumps(metrics, indent=2, sort_keys=True))
print(json.dumps(x['aggregate'], indent=2, sort_keys=True))
PY
