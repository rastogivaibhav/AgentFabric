  ...
  return (
    <div style={{ padding:32 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, marginBottom:28 }}>
        <StatCard
          label="TOTAL TRACES"
          value={isLoading ? '—' : (overview?.total_traces ?? 0).toLocaleString()}
          color="#3B82F6"
        />
        <StatCard
          label="TOTAL COST"
          value={isLoading ? '—' : `$${(overview?.total_cost_usd ?? 0).toFixed(4)}`}
          color="#F59E0B"
        />
        <StatCard
          label="AVG LATENCY"
          value={isLoading ? '—' : `${(overview?.avg_latency_ms ?? 0).toFixed(0)}ms`}
          color="#10B981"
        />
        <StatCard
          label="ERROR RATE"
          value={isLoading ? '—' : `${((overview?.error_rate ?? 0) * 100).toFixed(2)}%`}
          color={(overview?.error_rate ?? 0) > 0.05 ? '#EF4444' : '#10B981'}
        />
      </div>
        ...
        <div style={{ background:'#0D1B2A', border:'1px solid #0F1F35', borderRadius:10, padding:24 }}>
          ...
          <ResponsiveContainer width="100%" height={120}>
            <BarChart data={frameworkData}>
              ...
              <Tooltip
                contentStyle={{ background:'#0D1B2A', border:'1px solid #1E3A5F', borderRadius:6, fontSize:11 }}
              />
          ...
          <tbody>
            {(traces?.items ?? []).map((t, i) => (
              <tr key={t.id} style={{ background: i % 2 === 0 ? 'transparent' : '#060A1430' }}>
                ...
                <td style={{ padding:'8px 16px', color:'#94A3B8' }}>
                  {(t.duration_ns / 1_000_000).toFixed(0)}ms
                </td>
