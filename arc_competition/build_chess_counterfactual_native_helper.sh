...
// Plugin-only retrieval vectors. Counterfactual comparison anchors occupy a
// dedicated direction; all other nodes occupy a separate direction. This changes
// only the generated experiment helper under /tmp, never GrapheneDB/HypoKosh core.
std::vector<float> chess_plugin_node_vector(const RequestNode& node) {
  std::vector<float> out(kDimension, 0.0f);
  if (node.external_id.find("cf2-ply-") == 0 &&
      node.external_id.find("-comparison-anchor") != std::string::npos) {
    out[0] = 1.0f;
  } else {
    out[1] = 1.0f;
  }
...
std::vector<float> chess_plugin_query_vector() {
  std::vector<float> out(kDimension, 0.0f);
  out[0] = 1.0f;
  return out;
}
...
s=s.replace(old,'runtime.reason(chess_plugin_query_vector(), signature_for(1, 1), options);',1)
p.write_text(s)
PY

grep -q 'QueryMode::Theoretical' "$SRC"
grep -q 'chess_plugin_node_vector' "$SRC"
grep -q 'chess_plugin_query_vector' "$SRC"
