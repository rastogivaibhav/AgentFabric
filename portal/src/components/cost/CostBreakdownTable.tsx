    ...
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
      <thead>
        <tr style={{ borderBottom: '1px solid #0F1F35' }}>
          {['App', 'Environment', 'Provider', 'Model', 'Tokens', 'Input', 'Output', 'Cache Read', 'Cache Write', 'Reasoning', 'Total'].map(h => (
      ...
      <tbody>
        {rows.map((row, i) => {
          const tokenSummary = `${row.input_tokens + row.output_tokens + row.cache_read_tokens + row.cache_write_tokens + row.reasoning_tokens}`
          return (
